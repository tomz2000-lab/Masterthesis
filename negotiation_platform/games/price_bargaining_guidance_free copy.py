...
class CompanyCarGame(BaseGame):
    ...
    def _is_template_response(self, response: str) -> bool:
        """Check if response contains template/instructional text instead of actual content."""
        template_indicators = [
            "please respond",
            "note that",
            "please note",
        ...
        template_count = sum(1 for indicator in template_indicators if indicator in response_lower)
        
        # Also check for responses that are mostly instructional
        has_actual_decision = ("<reasoning>" in response_lower and 
                              "<decision>" in response_lower and 
                              "type" in response_lower)
        
        # If response has multiple template indicators or lacks proper structure, it's likely template text
        return (template_count >= 2) or (not has_actual_decision and template_count >= 1)
    ...
    def parse_structured_response(self, response: str) -> Dict[str, Any]:
        ...
        # Clean the response first by removing common instruction patterns
        cleaned_response = response
        instruction_patterns = [
            r"Please respond.*?(?=<REASONING>|$)",
            r"Example response.*?(?=<REASONING>|$)", 
            r"Note that.*?(?=<REASONING>|$)",
            r"Your response:.*?(?=<REASONING>|$)",
            r"```.*?(?=<REASONING>|$)",
            r"===.*?(?=<REASONING>|$)"
        ]
        ...
        if self._is_template_response(cleaned_response):
            print(f"⚠️ Detected template response, treating as reject")
            return {
                "reasoning": "Template response detected",
                "decision": {"type": "reject"},
                "message": "Could not generate proper response",
                "raw_response": response
            }
        
        try:
            # Extract reasoning - be more flexible with whitespace and newlines
            reasoning_match = re.search(r'<REASONING>\s*(.*?)\s*</REASONING>', cleaned_response, re.DOTALL)
            reasoning = reasoning_match.group(1).strip() if reasoning_match else ""
            
            # Extract decision (JSON) - be more flexible
            decision_match = re.search(r'<DECISION>\s*(.*?)\s*</DECISION>', cleaned_response, re.DOTALL)
            decision_text = decision_match.group(1).strip() if decision_match else ""
            
            # Extract message
            message_match = re.search(r'<MESSAGE>\s*(.*?)\s*</MESSAGE>', cleaned_response, re.DOTALL)
            message = message_match.group(1).strip() if message_match else ""
            
            # Parse JSON decision with better error handling
            decision_data = {"type": "reject"}  # Default fallback
            
            if decision_text and decision_text.strip():
                # Try to find JSON object in the decision text
                json_match = re.search(r'\{[^}]*"type"[^}]*\}', decision_text)
                if json_match:
                    ...
                    try:
                        ...
                        # Try to extract just the type and price manually
                        type_match = re.search(r'"type":\s*"([^"]+)"', decision_text)
                        price_match = re.search(r'"price":\s*(\d+)', decision_text)
                        if type_match:
                            decision_data = {"type": type_match.group(1)}
                            if price_match:
                                decision_data["price"] = int(price_match.group(1))
                else:
                    print(f"⚠️ No JSON found in decision text: {decision_text}")
            else:
                print(f"⚠️ Empty decision text, defaulting to reject")
    ...
    def get_game_prompt(self, player_id: str) -> str:
        ...
        prompt = f"""CAR NEGOTIATION - Round {current_round}/{self.max_rounds}
...
<REASONING>
[Your strategic thinking and analysis]
</REASONING>
